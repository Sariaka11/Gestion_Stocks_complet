...
function UserBienConsommation() {
  ...
  const handleAddModalSubmit = async (e) => {
    e.preventDefault();
    try {
      const parsedQuantiteConso = parseFloat(addFormData.quantiteConso);
  ...
  const handleDemande = async (bienId, nomBien) => {
    try {
      ...
      console.log("Envoi de la demande de notification pour:", { bienId, nomBien, userId: user.id, agenceId: userAgenceId });
      ...
      alert(`Demande envoyée pour ${nomBien} !`);
    } catch (error) {
              ...
              <tr key={item.id}>
                ...
                <td className="actions-cell">
                  ...
                  <button
                    ...
                    onClick={() => handleDemande(item.bienId, item.nomBien)}
                  >
                    Demande
                  </button>
